...
이 skill 디렉터리(`/Users/marshallku/.claude/skills/tabd`) 안에 docs 4개가 함께 install됨. Read 도구로 직접 접근:

- `Read /Users/marshallku/.claude/skills/tabd/commands.md` — 39개 액션의 옵션 / 응답 shape (가장 자주 lookup)
- `Read /Users/marshallku/.claude/skills/tabd/cookbook.md` — 시나리오 코드 패턴 (login + 2FA, API 캡쳐 3패턴, 세션 저장/복원, 무한 스크롤, CI 격리 daemon)
- `Read /Users/marshallku/.claude/skills/tabd/operations.md` — daemon 자체 운영 (systemd / launchd / drain / health 모니터)
- `Read /Users/marshallku/.claude/skills/tabd/architecture.md` — 왜 이렇게 설계됐는지 (multi-tab registry, reader-task deadlock, supervisor 등)
