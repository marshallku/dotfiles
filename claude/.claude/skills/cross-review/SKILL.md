...
- 구현성 변경이 있는 작업을 **끝내기 직전** (커밋/`~/save.sh` 전)
- 복잡하거나 보안 민감한 수정
...
Round 1은 **`--resume` 없이** 실행 — fresh thread를 띄운다. 이후 라운드가 이 thread를 재사용해서 codex가 round 1의 diff·분석·intent를 그대로 들고 가므로, 바뀐 부분만 추론하면 됨 (토큰 절감의 핵심).
...
AUTO-FIX 적용 후 **Round 1과 동일한 명령에 `--resume`만 추가해서** 재실행한다. `--resume`는 직전 review thread를 재사용하므로 diff/context/intent를 처음부터 다시 보내지 않고, codex는 적용된 fix만 검증한다:

```bash
# Round 1과 동일한 인자 + --resume
